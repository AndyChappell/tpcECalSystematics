{
	...
	// This doesn't use the combined neutrino / anti-neutrino method as the method has not been peer reviewed.
	...
	int nnu = 4; // Four neutrino types
	double nuBins[nnu + 1] = {-14.5, -13, 0, 13, 14.5};

	int ndet = 2; // Barrel and DS
	...
	for(int inu=0; inu<nnu; inu++) // Loop over neutrino type.
	{
		...
		for(int idet=0; idet<ndet; idet++) 
		{
			...
			if(inu == 1 || inu == 2) signal += isEl;
			else signal += isMu;

			vector<double> mc_lerr(nx);
			vector<double> mc_herr(nx);
			vector<double> mc_eff = draw.GetEfficiencyVsBin(mc, momentum, signal, cut[idet], nx, xbins, &mc_lerr, &mc_herr);

			vector<double> data_lerr(nx);
			vector<double> data_herr(nx);
			vector<double> data_eff = draw.GetEfficiencyVsBin(data, momentum, signal, cut[idet], nx, xbins, &data_lerr, &data_herr);
